  ...
  submitForm(): void {
    ...
    if (this.allFormGroupsValid()) {
      const formData = {
        ...
        nationalité: this.thirdFormGroup.get('nationalité')?.value,
        ...
        cinFront: this.cinFrontPreview ? (this.cinFrontPreview as string).split(',')[1] : '',
        cinBack: this.cinBackPreview ? (this.cinBackPreview as string).split(',')[1] : ''
      };

      console.log('FormData:', formData);

      this.formDataService.submitFormData(formData).subscribe(
        response => console.log('Données envoyées avec succès', response),
        error => console.error('Erreur lors de l\'envoi des données', error)
      );
    } else {
